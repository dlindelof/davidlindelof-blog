...
Here I show how with this function one can derive a contour plot showing, for given clothing and metabolism, the optimal indoor temperature (assuming 50% relative humidity). We're basically going to solve the `pmv(clo, met, temp, sat) = 0` equation for `temp` across a grid of `clo` and `met` values with the `uniroot` function.

```r
clo <- seq(0, 2, length = 21)
met <- seq(0.6, 3.2, length = 21)

zero.pmv <- function(clo, met) {
  uniroot(function(temp) pmv(clo, met, temp, 50), c(-10, 40))$root
}

contourplot(
  outer(clo, met, Vectorize(zero.pmv)),
  cuts = 20,
  aspect = "fill",
  panel = function(...) {
    panel.grid(h = -1, v = -1, ...)
  ...
  row.values = clo,
  column.values = met,
  xlim = extendrange(clo),
  ylim = extendrange(met),
  xlab = "[Clo]",
  ylab = "[Met]"
)
```
